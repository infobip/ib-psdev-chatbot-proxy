...
async function sessionLookup(conversationId) {
  logger.debug(
    `sessionLookup: Here in sessionLookup for ${conversationId} ...`
  );
  var was;
  ...
  if (sessionsTable.hasOwnProperty(conversationId)) {
    ...
    if (was) {
      logger.debug(
        `sessionLookup: Database lookup for ${conversationId} returned session record: ` +
          JSON.stringify(was)
      );
  ...
  if (!was) {
    logger.debug(
      `sessionLookup: Conversation ${conversationId} was not found in our sessionsTable`
    );
    return null;
  }
  ...
  if (now_time > was["expiry"]) {
    ...
    logger.debug(
      `sessionLookup: Conversation ${conversationId} points to an expired session_id ${session_id}`
    );
    return null;
  } else {
    logger.debug(
      `sessionLookup: Conversation ${conversationId} found and session_id ${session_id} can be resumed`
    );
    return was;
  }
}
...
async function sessionCreate(conversationId, was) {
  ...
  logger.debug(
    `sessionCreate: Session entry created for ${conversationId}: ` +
      JSON.stringify(was)
  );
  return was;
}
...
async function assistant_createSession(conversationId, fromAddress) {
  var was;
  logger.debug(
    `assistant_createSession: Session creation requested for conversation ${conversationId}`
  );
  await IWA_assistantV2.createSession({ assistantId: IWA_assistantId })
    .then(async (res) => {
      ...
      logger.debug(
        `assistant_createSession: Session ${session_id} created for conversation ${conversationId}\n` +
          `Watson createSession Response: ` +
          JSON.stringify(res.result)
      );
    })
    .catch((err) => {
      logger.error(
        `Failed in assistant_createSession() for conversation ${conversationId}: `,
        err
      );
      throw Error(err);
    });
...
async function assistant_deleteSession(conversationId, session_id) {
  logger.debug(
    `assistant_deleteSession: Session deletion requested for conversation ${conversationId}`
  );
  if (!session_id) {
    var was = await sessionLookup(conversationId);
    session_id = was.session_id;
  }
  await IWA_assistantV2.deleteSession({
    ...
    .then(async (res) => {
      sessionTerminate(conversationId);
      logger.debug(
        `assistant_deleteSession: Session ${session_id} deleted for conversation ${conversationId}\n` +
          `Watson deleteSession Response: ` +
          JSON.stringify(res.result)
      );
    })
    .catch((err) => {
      logger.error(
        `Failed in assistant_deleteSession() for conversation ${conversationId} / session ${session_id}: `,
        err
      );
      throw Error(err);
    });
...
async function assistant_exchangeMessage(ibMessageObj, textMsg) {
  ...
  logger.debug(
    `assistant_exchangeMessage: Message from conversation ${conversationId} = '${textMsg}' queued for delivery to Watson`
  );
  var was = await sessionLookup(conversationId);
  logger.debug(
    `assistant_exchangeMessage: sessionLookup(${conversationId}) returned ${was}`
  );
  var result;
  if (!(was && was.hasOwnProperty("session_id"))) {
    logger.debug(
      `assistant_exchangeMessage: calling assistant_createSession() for conversation ${conversationId}`
    );
    was = await assistant_createSession(conversationId, ibMessageObj.from);
  }
  logger.debug(
    `got session object: ${was} = ` + (was ? JSON.stringify(was) : "null")
  );
  if (!(was && was.hasOwnProperty("session_id"))) {
  ...
  await IWA_assistantV2.message({
    ...
    .catch((err) => {
      logger.error(
        `Failed in assistant_exchangeMessage() for conversation ${conversationId}: `,
        err
      );
      throw Error(err);
    });
...
function wat_serialize_options(genericItem) {
  if (
    genericItem.hasOwnProperty("options") &&
    genericItem.hasOwnProperty("title")
  ) {
    ...
    for (const key in options_in) {
      if (options_in[key].hasOwnProperty("label")) {
        ...
        opt_input_suggest = opt_input_suggest
          .replace(/\s+/g, " ")
          .replace(/^\s*|\s*$/g, "");
    ...
    for (const key in options_in) {
      if (options_in[key].hasOwnProperty("label")) {
        ...
        options_out.push({
          title: oneopt["label"],
          type: "POSTBACK",
          payload: oneopt["value"].input.text
        });
      }
    }
