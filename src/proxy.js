// Main ChatBot Proxy application source code file
...
const {
  sessionLookup,
  sessionTerminate,
  assistant_exchangeMessage,
  assistant_deleteSession,
  wat_serialize_options,
  wat_button_widget
} = require("./assistant");
...
const getBotIds = async () => {
  ...
  try {
    ...
    logger.error(
      "Error while getting bots identifiers from Infobip Conversations",
      err
    );
    throw Error(err);
  }
...
function convPostMessageTextSingle(xResponse, ibMessageObj) {
  ...
  var post_url;
  post_url =
    config.get("BIP_serviceURL") +
    "/ccaas/1/conversations/" +
    conversationId +
    "/messages";
  logger.debug(
    xResponse.locals.flowControlId + ` Preparing the POST call to: ${post_url}`
  );
  ...
  logger.debug(
    xResponse.locals.flowControlId + ` Performing the POST call to: ${post_url}`
  );

  // const response =
  axios
    .post(post_url, ibMessageObj, { headers: bipHeadersPostMsg })
    .then((res) => {
      logger.debug(
        xResponse.locals.flowControlId +
          ` POST->message result status: ` +
          res.status
      );
      return true;
    })
    .catch((err) => {
      if (err.response) {
        logger.error(
          `Server Error while returning reply message on conversation ${conversationId}: `,
          err
        );
      } else {
        logger.error(
          `Error while routing conversation ${conversationId} on Infobip Conversations: `,
          err
        );
        throw Error(err);
      }
...
async function convPostMessageMany(
  xResponse,
  conversationId,
  channel,
  replyScenario
  ...
  logger.info(
    xResponse.locals.flowControlId +
      " Reply scenario with " +
      replyScenario.length +
      " parts prepared for delivery"
  );
  ...
  for await (const replyItem of replyScenario) {
    ...
    logger.debug(
      xResponse.locals.flowControlId +
        ` Handling reply scenarion item ${itemSeqNo}: ` +
    ...
    if (Step.REPLY === replyItem.step) {
      ...
      try {
        logger.debug(
          xResponse.locals.flowControlId + ` attemptint to POST->message(...)`
        );
        const response = await axios.post(post_url, ibMessageObj, {
          headers: bipHeadersPostMsg
        });
        logger.debug(
          xResponse.locals.flowControlId +
            ` POST->message(...) result status: ` +
        ...
        if (err.response) {
          logger.error(
            `Server Error while messaging conversation ${conversationId} on Infobip Conversations: `,
            err
          );
        } else {
          logger.error(
            `Error while messaging conversation ${conversationId} on Infobip Conversations: `,
            err
          );
          throw Error(err);
        }
...
async function convRouteConv(xResponse, conversationId, tagsArray) {
  ...
  logger.debug(
    xResponse.locals.flowControlId +
      ` Preparing the POST call(s) to: ${post_url}`
  );
  ...
  for await (const tagName of tagsArray) {
    try {
      logger.debug(
        xResponse.locals.flowControlId + ` attemptint to POST->tags(${tagName})`
      );
      const response = await axios.post(
        post_url,
        { tagName: tagName },
        { headers: bipHeadersPost }
      );
      logger.debug(
        xResponse.locals.flowControlId +
          ` POST->tags(${tagName}) result status: ` +
          response.status
      );
    } catch (err) {
      noErrorHappened = false;
      if (err.response) {
        logger.error(
          `Server Error while tagging conversation ${conversationId} on Infobip Conversations: `,
          err
        );
      } else {
        logger.error(
          `Error while tagging conversation ${conversationId} on Infobip Conversations: `,
          err
        );
        throw Error(err);
      }
  ...
  if (noErrorHappened) {
    try {
      ...
      logger.debug(
        xResponse.locals.flowControlId +
          ` attemptint to POST->route call to: ${post_url}`
      );
      const response = await axios.post(
        post_url,
        {},
      ...
      if (err.response) {
        logger.error(
          `Server Error while routing conversation ${conversationId} on Infobip Conversations: `,
          err
        );
      } else {
        logger.error(
...
const initMain = async () => {
  try {
    ...
    if (myBotId_LC || myBotId_WA) {
      ...
      if (myBotId_LC)
        logger.info(
          `Bot.ID for LiveChat = ${myBotId_LC}, Bot.displayName = ${myBotName_LC}`
        );
      if (myBotId_WA)
        logger.info(
          `Bot.ID for WhatsApp = ${myBotId_WA}, Bot.displayName = ${myBotName_WA}`
        );
    } else {
      logger.error(
        "FALIURE: couldn't get any active botId from Infobip Conversations"
      );
      process.exit(1);
    }
    ...
    app.listen(appPort, appHost, () => {
      logger.info(
        `Application ${appName} listening at http://${appHost}:${appPort}/`
      );
    });
  } catch (err) {
    console.error(err);
...
app.get("/", (req, res) => {
  ...
  res.send(
    `Hello client connecting from ${ip}, this is "${appName}", up since ${appStartupTime}, reporting at ${tstmp}`
  );
});

// main handler of POST / requests
app.post("/", async (xRequest, xResponse) => {
  var ip = xRequest.headers["x-forwarded-for"] || xRequest.socket.remoteAddress;
  logger.info(
    `Received POST / request from client at ${ip} flow control Id: ` +
      xResponse.locals.flowControlId
  );
  logger.debug(
    "POST request [" +
  ...
  if (inboundTextMsg.match(RE_SWA_KWord)) {
    logger.info(
      xResponse.locals.flowControlId +
        ` Registered keyword '${SWA_KWord}' received on ${existingOrNew} conversationId: ${conversationId}`
    );
    inboundTextMsg = inboundTextMsg.replace(RE_SWA_KWord, "");
    if (inboundTextMsg.length === 0) {
      logger.info(
        xResponse.locals.flowControlId +
          `  ... as it is the only word received, responding with session status message`
      );
      if (was) {
        forwardTextMsg =
          `This conversation is connected to Watson Assistant via ${appName} already.\n` +
  ...
  if (inboundTextMsg.toUpperCase() === "STOP") {
    logger.info(
      xResponse.locals.flowControlId +
        ` STOP received on ${existingOrNew} conversationId: ${conversationId}`
    );
    ...
    inboundTextMsg.toUpperCase() === "LOOPTEST" ||
    inboundTextMsg.match(RE_SWA_KWord)
  ) {
    logger.info(
      xResponse.locals.flowControlId +
        ` LOOPTEST or KEYWORD received on this conversationId: ${conversationId}`
    );
    ...
    logger.info(
      xResponse.locals.flowControlId +
        ` Regular message received on ${existingOrNew} conversationId: ${conversationId}`
    );
  ...
  if (nextStep === Step.WATSON) {
    ...
    logger.info(
      xResponse.locals.flowControlId +
        ` We've got Watson Assistant's response: ` +
        JSON.stringify(aMsg)
    );

    if (aMsg) {
      // aMsg ~ response.result
      ...
      logger.info(
        xResponse.locals.flowControlId +
          ` analyzing Watson Assistant's response...`
      );
      for await (const ogi of rog) {
        if (ogi.response_type === "text") {
          ...
          forwardTextMsg =
            "(response type '" + ogi.response_type + "' not supported yet)";
          nextStep = Step.REPLY;
        }
        // ----
      ...
      if (inboundTextMsg.match("agent|route")) {
        forwardTextMsg =
          "Virtual Assistant is momentarily not available. Chatbot Proxy will route this conversation to a Human Agent.";
        nextStep = Step.ROUTE;
      } else {
        // register session ...
        forwardTextMsg =
          "Virtual Assistant is momentarily not available. Please respond with 'agent' to have this conversation routed to a Human Agent.";
        nextStep = Step.REPLY;
      }
    }
  }
  ...
  logger.debug(
    xResponse.locals.flowControlId +
      ` forwardTextMsg=${forwardTextMsg} allResponses#=${nreplies} nextStep=${nextStep}`
  );
  logger.info(
    xResponse.locals.flowControlId +
      ` allResponses: \n` +
  ...
  if (Step.ROUTE === nextStep) {
    if (await convRouteConv(xResponse, conversationId, ["fromWatson"])) {
      logger.info(
        xResponse.locals.flowControlId +
          ` Tagging and routing of conversationId: ${conversationId} was successful.`
      );
    } else {
      logger.error(
        xResponse.locals.flowControlId +
          ` Tagging and routing of conversationId: ${conversationId} was unsuccessful!`
      );
    }
  }
  ...
  if (Step.STOP === nextStep) {
    ...
    logger.info(
      `For conversationId: ${conversationId} session terminated on STOP keyword.`
    );
  }

  logger.info(
    xResponse.locals.flowControlId + " Done processing POST / request."
  );
});
